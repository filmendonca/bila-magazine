...
class classes_Database{
	...
	private $_myDb = null;
	private $_configFilePath = "config/dbconfig.ini";
	...
	public function __construct(){
		...
		if ( !file_exists( $this->_configFilePath ) ){
            ...
            $dbConfigElements = parse_ini_file($this->_configFilePath);
	...
	public function executeQuery( $query, $orderedParameters){
		...
		if ( !is_string($query) || !is_array($orderedParameters) ){
			...
			try{
				...
				//Se a palavra LIKE estiver na string
				if(strpos($query, "LIKE") != false){

					foreach ($orderedParameters as $key => &$value) {
						$value = "%".$value."%";
                ...
                $check = array_search(null, $orderedParameters);     
				...
				if($check == "id"){
					...
					$statement->execute($orderedParameters);
				}

				

				//$statement->bindValue(':titulo', '%' . $orderedParameters["titulo"] .'%');
				//$statement->bindValue(':texto', '%' . $orderedParameters["texto"] . '%');

				

				//$statement->execute();
				
				if ( !$statement->execute() && !$statement->execute($orderedParameters)){
